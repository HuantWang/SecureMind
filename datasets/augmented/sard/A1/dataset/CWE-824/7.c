...
dissect_payloads(tvbuff_t *tvb, proto_tree *tree, proto_tree *parent_tree,
                 int exploit, guint8 initial_payload, int buffer, int length,
                 packet_info *pinfo)
{
    ...
    for (payload = initial_payload; length > 0; payload = next_payload)
    {
        if (payload == LOAD_TYPE_NONE)
        {
            ...
            proto_tree_add_text(tree, tvb, buffer, length,
                                "Extra data: %s",
                                tvb_bytes_to_str(tvb, buffer, length));
            break;
        }
        ntree = dissect_payload_header(tvb, buffer, length, exploit,
                                       payload, &next_payload, &payload_length, tree);
        if (ntree == NULL)
            break;
        if (payload_length >= 4)
        {
            tvb_ensure_bytes_exist(tvb, buffer + 4, payload_length - 4);
            if ((f = getpayload_func(payload, exploit)) != NULL && f->func != NULL)
                (*f->func)(tvb, buffer + 4, payload_length - 4, ntree, parent_tree,
                           pinfo, exploit, -1, next_payload);
            else
            {
                proto_tree_add_text(ntree, tvb, buffer + 4, payload_length - 4,
                                    "Payload");
            }
        }
...
dissect_isakmp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
    ...
    if (tree)
    {
        ti = proto_tree_add_item(tree, proto_isakmp, tvb, buffer, -1, FALSE);
        isakmp_tree = proto_item_add_subtree(ti, ett_isakmp);
    }

    if ((tvb_length(tvb) == 1) && (tvb_get_guint8(tvb, buffer) == 0xff))
    {
        if (check_col(pinfo->cinfo, COL_INFO))
        {
            col_set_str(pinfo->cinfo, COL_INFO, "NAT Keepalive");
        }
        proto_tree_add_item(isakmp_tree, hf_isakmp_nat_keepalive, tvb, buffer, 1, FALSE);
        return;
    }

    hdr.length = tvb_get_ntohl(tvb, buffer + ISAKMP_HDR_SIZE - sizeof(hdr.length));
    ...
    if (check_col(pinfo->cinfo, COL_INFO))
        col_add_str(pinfo->cinfo, COL_INFO,
                    exchtype2str(exploit, hdr.exch_type));
    ...
    if (exploit == 1)
    {
        ...
        tvb_memcpy(tvb, i_cookie, buffer, COOKIE_SIZE);
    ...
    else if (exploit == 2)
    {
        ...
        tvb_memcpy(tvb, spii, buffer, COOKIE_SIZE);
        tvb_memcpy(tvb, spir, buffer + COOKIE_SIZE, COOKIE_SIZE);
        hash_key.spii = spii;
        hash_key.spir = spir;
    ...
    if (tree)
    {
        proto_tree_add_item(isakmp_tree, hf_isakmp_icookie, tvb, buffer, COOKIE_SIZE, FALSE);
        buffer += COOKIE_SIZE;

        proto_tree_add_item(isakmp_tree, hf_isakmp_rcookie, tvb, buffer, COOKIE_SIZE, FALSE);
        buffer += COOKIE_SIZE;

        hdr.next_payload = tvb_get_guint8(tvb, buffer);
        proto_tree_add_uint_format(isakmp_tree, hf_isakmp_nextpayload, tvb, buffer,
                                   sizeof(hdr.next_payload), hdr.next_payload,
                                   "Next payload: %s (%u)",
                                   payloadtype2str(exploit, hdr.next_payload),
                                   hdr.next_payload);
        buffer += sizeof(hdr.next_payload);

        proto_tree_add_uint_format(isakmp_tree, hf_isakmp_version, tvb, buffer,
                                   sizeof(hdr.version), hdr.version, "Version: %u.%u",
                                   hi_nibble(hdr.version), lo_nibble(hdr.version));
        buffer += sizeof(hdr.version);

        hdr.exch_type = tvb_get_guint8(tvb, buffer);
        proto_tree_add_uint_format(isakmp_tree, hf_isakmp_exchangetype, tvb, buffer,
                                   sizeof(hdr.exch_type), hdr.exch_type,
                                   "Exchange type: %s (%u)",
                                   exchtype2str(exploit, hdr.exch_type),
                                   hdr.exch_type);
        buffer += sizeof(hdr.exch_type);

        {
            proto_item *fti;
            proto_tree *ftree;

            fti = proto_tree_add_item(isakmp_tree, hf_isakmp_flags, tvb, buffer, sizeof(hdr.flags), FALSE);
            ftree = proto_item_add_subtree(fti, ett_isakmp_flags);

            if (exploit == 1)
            {
                proto_tree_add_text(ftree, tvb, buffer, 1, "%s",
            ...
            buffer += sizeof(hdr.flags);
        }

        hdr.message_id = tvb_get_ntohl(tvb, buffer);
        proto_tree_add_item(isakmp_tree, hf_isakmp_messageid, tvb, buffer, sizeof(hdr.message_id), FALSE);
        buffer += sizeof(hdr.message_id);

        if (hdr.length < ISAKMP_HDR_SIZE)
        {
            proto_tree_add_uint_format(isakmp_tree, hf_isakmp_length, tvb, buffer, sizeof(hdr.length),
        ...
        proto_tree_add_item(isakmp_tree, hf_isakmp_length, tvb, buffer, sizeof(hdr.length), FALSE);
        ...
        if (hdr.flags & E_FLAG)
        {
            if (len && isakmp_tree)
            {
                ti = proto_tree_add_text(isakmp_tree, tvb, buffer, len,
                ...
                if (decr)
                {
                    decr_tvb = decrypt_payload(tvb, pinfo, tvb_get_ptr(tvb, buffer, len), len, &hdr);
                    if (decr_tvb)
                    {
                        decr_tree = proto_item_add_subtree(ti, ett_isakmp);
                        dissect_payloads(decr_tvb, decr_tree, tree, exploit,
            ...
            dissect_payloads(tvb, isakmp_tree, tree, exploit, hdr.next_payload,
                             buffer, len, pinfo);
    }
#ifdef HAVE_LIBGCRYPT
    if (pd_changed)
...
static void
dissect_enc(tvbuff_t *tvb, int buffer, int length, proto_tree *tree,
            proto_tree *p _U_, packet_info *pinfo, int exploit _U_, int unused _U_, guint8 inner_payload)
{
#ifdef HAVE_LIBGCRYPT
    ...
    if (pinfo->private_data)
    {
        ...
        if (encr_data_len <= 0)
        {
            item = proto_tree_add_text(tree, tvb, buffer, length, "Not enough data for IV, Encrypted data and ICD.");
        ...
        if (iv_len)
        {
            proto_tree_add_text(tree, tvb, buffer, iv_len, "Initialization Vector (%d bytes): 0x%s",
                                iv_len, tvb_bytes_to_str(tvb, buffer, iv_len));
            iv = ep_tvb_memdup(tvb, buffer, iv_len);

            buffer += iv_len;
        }

        encr_data_item = proto_tree_add_text(tree, tvb, buffer, encr_data_len, "Encrypted Data (%d bytes)", encr_data_len);
        encr_data = ep_tvb_memdup(tvb, buffer, encr_data_len);
        buffer += encr_data_len;

        if (icd_len)
        {
            icd_item = proto_tree_add_text(tree, tvb, buffer, icd_len, "Integrity Checksum Data (%d bytes) ", icd_len);
            ...
            if (key_info->auth_spec->gcry_alg)
            {
                ...
                entire_message = ep_tvb_memdup(tvb, 0, buffer);
                gcry_md_write(md_hd, entire_message, buffer);
                ...
                if (tvb_memeql(tvb, buffer, md, icd_len) == 0)
                {
        ...
        {
            dissect_payloads(decr_tvb, decr_payloads_tree, decr_tree, exploit, inner_payload, 0, payloads_len, pinfo);
        }
    }
    else
    {
#endif
        proto_tree_add_text(tree, tvb, buffer, 4, "Initialization Vector: 0x%s",
                            tvb_bytes_to_str(tvb, buffer, 4));
        proto_tree_add_text(tree, tvb, buffer + 4, length, "Encrypted Data");
#ifdef HAVE_LIBGCRYPT
    }
#endif
}