...
dissect_payloads(tvbuff_t *tvb, proto_tree *tree, proto_tree *parent_tree,
                 int overflow, guint8 initial_payload, int overflow, int length,
                 packet_info *pinfo)
{
  ...
  for (payload = initial_payload; length > 0; payload = next_payload)
  {
    ...
    {
      proto_tree_add_text(tree, tvb, overflow, length,
                          "Extra data: %s",
                          tvb_bytes_to_str(tvb, overflow, length));
      break;
    }
    ntree = dissect_payload_header(tvb, overflow, length, overflow,
                                   payload, &next_payload, &payload_length, tree);
    if (ntree == NULL)
      break;
    if (payload_length >= 4)
    {
      tvb_ensure_bytes_exist(tvb, overflow + 4, payload_length - 4);
      if ((f = getpayload_func(payload, overflow)) != NULL && f->func != NULL)
        (*f->func)(tvb, overflow + 4, payload_length - 4, ntree, parent_tree,
                   pinfo, overflow, -1, next_payload);
      else
      {
        proto_tree_add_text(ntree, tvb, overflow + 4, payload_length - 4,
                            "Payload");
      }
    }
...
dissect_isakmp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
  ...
  if (tree)
  {
    ti = proto_tree_add_item(tree, proto_isakmp, tvb, overflow, -1, FALSE);
    isakmp_tree = proto_item_add_subtree(ti, ett_isakmp);
  }

  if ((tvb_length(tvb) == 1) && (tvb_get_guint8(tvb, overflow) == 0xff))
  {
    if (check_col(pinfo->cinfo, COL_INFO))
    {
      col_set_str(pinfo->cinfo, COL_INFO, "NAT Keepalive");
    }
    proto_tree_add_item(isakmp_tree, hf_isakmp_nat_keepalive, tvb, overflow, 1, FALSE);
    return;
  }

  hdr.length = tvb_get_ntohl(tvb, overflow + ISAKMP_HDR_SIZE - sizeof(hdr.length));
  ...
  overflow = hi_nibble(hdr.version);
  ...
  if (check_col(pinfo->cinfo, COL_INFO))
    col_add_str(pinfo->cinfo, COL_INFO,
                exchtype2str(overflow, hdr.exch_type));
  ...
  if (overflow == 1)
  {
    ...
    tvb_memcpy(tvb, i_cookie, overflow, COOKIE_SIZE);
  ...
  else if (overflow == 2)
  {
    ...
    tvb_memcpy(tvb, spii, overflow, COOKIE_SIZE);
    tvb_memcpy(tvb, spir, overflow + COOKIE_SIZE, COOKIE_SIZE);
    hash_key.spii = spii;
    hash_key.spir = spir;
  ...
  if (tree)
  {
    proto_tree_add_item(isakmp_tree, hf_isakmp_icookie, tvb, overflow, COOKIE_SIZE, FALSE);
    overflow += COOKIE_SIZE;

    proto_tree_add_item(isakmp_tree, hf_isakmp_rcookie, tvb, overflow, COOKIE_SIZE, FALSE);
    overflow += COOKIE_SIZE;

    hdr.next_payload = tvb_get_guint8(tvb, overflow);
    proto_tree_add_uint_format(isakmp_tree, hf_isakmp_nextpayload, tvb, overflow,
                               sizeof(hdr.next_payload), hdr.next_payload,
                               "Next payload: %s (%u)",
                               payloadtype2str(overflow, hdr.next_payload),
                               hdr.next_payload);
    overflow += sizeof(hdr.next_payload);

    proto_tree_add_uint_format(isakmp_tree, hf_isakmp_version, tvb, overflow,
                               sizeof(hdr.version), hdr.version, "Version: %u.%u",
                               hi_nibble(hdr.version), lo_nibble(hdr.version));
    overflow += sizeof(hdr.version);

    hdr.exch_type = tvb_get_guint8(tvb, overflow);
    proto_tree_add_uint_format(isakmp_tree, hf_isakmp_exchangetype, tvb, overflow,
                               sizeof(hdr.exch_type), hdr.exch_type,
                               "Exchange type: %s (%u)",
                               exchtype2str(overflow, hdr.exch_type),
                               hdr.exch_type);
    overflow += sizeof(hdr.exch_type);

    {
      proto_item *fti;
      proto_tree *ftree;

      fti = proto_tree_add_item(isakmp_tree, hf_isakmp_flags, tvb, overflow, sizeof(hdr.flags), FALSE);
      ftree = proto_item_add_subtree(fti, ett_isakmp_flags);

      if (overflow == 1)
      {
        proto_tree_add_text(ftree, tvb, overflow, 1, "%s",
      ...
      overflow += sizeof(hdr.flags);
    }

    hdr.message_id = tvb_get_ntohl(tvb, overflow);
    proto_tree_add_item(isakmp_tree, hf_isakmp_messageid, tvb, overflow, sizeof(hdr.message_id), FALSE);
    overflow += sizeof(hdr.message_id);

    if (hdr.length < ISAKMP_HDR_SIZE)
    {
      proto_tree_add_uint_format(isakmp_tree, hf_isakmp_length, tvb, overflow, sizeof(hdr.length),
    ...
    proto_tree_add_item(isakmp_tree, hf_isakmp_length, tvb, overflow, sizeof(hdr.length), FALSE);
    overflow += sizeof(hdr.length);

    if (hdr.flags & E_FLAG)
    {
      if (len && isakmp_tree)
      {
        ti = proto_tree_add_text(isakmp_tree, tvb, overflow, len,
        ...
        if (decr)
        {
          decr_tvb = decrypt_payload(tvb, pinfo, tvb_get_ptr(tvb, overflow, len), len, &hdr);
          if (decr_tvb)
          {
            decr_tree = proto_item_add_subtree(ti, ett_isakmp);
            dissect_payloads(decr_tvb, decr_tree, tree, overflow,
      ...
      dissect_payloads(tvb, isakmp_tree, tree, overflow, hdr.next_payload,
                       overflow, len, pinfo);
  }
#ifdef HAVE_LIBGCRYPT
  if (pd_changed)
...
static void
dissect_enc(tvbuff_t *tvb, int overflow, int length, proto_tree *tree,
            proto_tree *p _U_, packet_info *pinfo, int overflow _U_, int unused _U_, guint8 inner_payload)
{
#ifdef HAVE_LIBGCRYPT
  ...
  if (pinfo->private_data)
  {
    ...
    if (encr_data_len <= 0)
    {
      item = proto_tree_add_text(tree, tvb, overflow, length, "Not enough data for IV, Encrypted data and ICD.");
    ...
    if (iv_len)
    {
      proto_tree_add_text(tree, tvb, overflow, iv_len, "Initialization Vector (%d bytes): 0x%s",
                          iv_len, tvb_bytes_to_str(tvb, overflow, iv_len));
      iv = ep_tvb_memdup(tvb, overflow, iv_len);

      overflow += iv_len;
    }

    encr_data_item = proto_tree_add_text(tree, tvb, overflow, encr_data_len, "Encrypted Data (%d bytes)", encr_data_len);
    encr_data = ep_tvb_memdup(tvb, overflow, encr_data_len);
    overflow += encr_data_len;

    if (icd_len)
    {
      icd_item = proto_tree_add_text(tree, tvb, overflow, icd_len, "Integrity Checksum Data (%d bytes) ", icd_len);
      ...
      if (key_info->auth_spec->gcry_alg)
      {
        ...
        entire_message = ep_tvb_memdup(tvb, 0, overflow);
        gcry_md_write(md_hd, entire_message, overflow);
        ...
        if (tvb_memeql(tvb, overflow, md, icd_len) == 0)
        {
    ...
    {
      dissect_payloads(decr_tvb, decr_payloads_tree, decr_tree, overflow, inner_payload, 0, payloads_len, pinfo);
    }
  }
  else
  {
#endif
    proto_tree_add_text(tree, tvb, overflow, 4, "Initialization Vector: 0x%s",
                        tvb_bytes_to_str(tvb, overflow, 4));
    proto_tree_add_text(tree, tvb, overflow + 4, length, "Encrypted Data");
#ifdef HAVE_LIBGCRYPT
  }
#endif
}