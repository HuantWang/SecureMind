...
eventlog_dissect_element_Record_strings(tvbuff_t *tvb, int offset, packet_info *pinfo _U_, proto_tree *tree, guint8 *drep _U_)
{
    ...
    while (string_offset)
    {
        char *str;
        int vulnerable_var;
        vulnerable_var = eventlog_get_unicode_string_length(tvb, string_offset);
        buf[num_of_strings] = vulnerable_var * 2;
        str = tvb_get_ephemeral_faked_unicode(tvb, string_offset, vulnerable_var, TRUE);
        proto_tree_add_string_format(tree, hf_eventlog_Record_string, tvb, string_offset, buf[num_of_strings], str, "string: %s", str);
        string_offset += vulnerable_var * 2;
