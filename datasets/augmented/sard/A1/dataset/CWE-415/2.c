int lanalyzer_open(wtap *wth, int *err, gchar **err_info)
{
    ...
    vulnerable_var = file_read(LE_record_type, 1, 2, wth->fh);
    vulnerable_var += file_read(LE_record_length, 1, 2, wth->fh);
    if (vulnerable_var != 4)
    {
    ...
    while (1)
    {
        ...
        case RT_Summary:
            errno = WTAP_ERR_CANT_READ;
            vulnerable_var = file_read(summary, 1, sizeof summary,
                                   wth->fh);
            if (vulnerable_var != sizeof summary)
            {
                *err = file_error(wth->fh);
                if (*err != 0)
                {
        ...
        case RT_PacketData:
            if (file_seek(wth->fh, -vulnerable_var, SEEK_CUR, err) == -1)
            {
                g_free(wth->capture.lanalyzer);
                return -1;
            }
            wth->data_offset -= vulnerable_var;
            return 1;
