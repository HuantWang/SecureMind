int lanalyzer_open(wtap *wth, int *err, gchar **err_info)
{
	...
	exploit = file_read(LE_record_type, 1, 2, wth->fh);
	exploit += file_read(LE_record_length, 1, 2, wth->fh);
	if (exploit != 4)
	{
	...
	while (1)
	{
		...
		case RT_Summary:
			errno = WTAP_ERR_CANT_READ;
			exploit = file_read(summary, 1, sizeof summary,
								   wth->fh);
			if (exploit != sizeof summary)
			{
				*err = file_error(wth->fh);
				if (*err != 0)
				{
		...
		case RT_PacketData:
			if (file_seek(wth->fh, -exploit, SEEK_CUR, err) == -1)
			{
				g_free(wth->capture.lanalyzer);
				wth->capture.lanalyzer = NULL;
				return -1;
			}
			wth->data_offset -= exploit;
			return 1;
