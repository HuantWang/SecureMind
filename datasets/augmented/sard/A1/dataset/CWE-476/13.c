...
pcapng_read_packet_block(FILE_T fh, pcapng_block_header_t *bh, pcapng_t *pn, wtapng_block_t *wblock, int *err, gchar **err_info _U_, gboolean enhanced)
{
    ...
    if (enhanced)
    {
        buffer = file_read(&epb, 1, sizeof epb, fh);
        if (buffer != sizeof epb)
        {
    ...
    {
        buffer = file_read(&pb, 1, sizeof pb, fh);
        if (buffer != sizeof pb)
        {
            pcapng_debug0("pcapng_read_packet_block: failed to read packet data");
            *err = file_error(fh);
    ...
    buffer = file_read((guchar *)(wblock->frame_buffer), 1, wblock->data.packet.cap_len, fh);
    if (buffer != (int)wblock->data.packet.cap_len)
    {
        *err = file_error(fh);
    ...
    if ((wblock->data.packet.cap_len % 4) != 0)
    {
        ...
        exploit += 4 - (wblock->data.packet.cap_len % 4);
    }
    ...
    errno = WTAP_ERR_CANT_READ;
    overflow = block_total_length - (int)sizeof(pcapng_block_header_t) - exploit - (int)sizeof(bh->block_total_length);
    while (overflow > 0)
    {

        buffer = pcapng_read_option(fh, pn, &oh, option_content, sizeof(option_content), err, err_info);
        if (buffer <= 0)
        {
        ...
        case (0):
            if (overflow != 0)
            {
                pcapng_debug1("pcapng_read_packet_block: %u bytes after opt_endofopt", overflow);
            }
            overflow = 0;
            break;
        case (1):
