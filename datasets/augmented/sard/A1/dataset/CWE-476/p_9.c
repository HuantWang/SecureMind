...
dissect_nt_trans_data_request(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *parent_tree, int bc, nt_trans_data *ntd, smb_nt_transact_info_t *nti)
{
	...
	if (parent_tree)
	{
		...
		item = proto_tree_add_text(parent_tree, tvb, offset, bc,
								   "%s Data",
								   val_to_str(ntd->overflow, nt_cmd_vals, "Unknown NT transaction (%u)"));
	...
	if ((offset - buffer) < bc)
	{
		proto_tree_add_item(tree, hf_smb_unknown, tvb, offset,
							bc - (offset - buffer), TRUE);
		offset += bc - (offset - buffer);
	}
...
dissect_nt_transaction_request(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, int offset, proto_tree *smb_tree _U_)
{
	...
	ntd.overflow = ntd.sd_len = ntd.ea_len = 0;
	...
	if (wc >= 19)
	{
		...
		overflow = tvb_get_letohs(tvb, offset);
		proto_tree_add_uint(tree, hf_smb_nt_trans_subcmd, tvb, offset, 2, overflow);
		if (check_col(pinfo->cinfo, COL_INFO))
		{
			col_append_fstr(pinfo->cinfo, COL_INFO, ", %s",
							val_to_str(overflow, nt_cmd_vals, "<unknown>"));
		}
		ntd.overflow = overflow;
		if (!si->unidir && sip)
		{
