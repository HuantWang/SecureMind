...
pcapng_read_packet_block(FILE_T fh, pcapng_block_header_t *bh, pcapng_t *pn, wtapng_block_t *wblock, int *err, gchar **err_info _U_, gboolean enhanced)
{
	...
	if (enhanced)
	{
		overflow = file_read(&epb, 1, sizeof epb, fh);
		if (overflow != sizeof epb)
		{
		...
		{
			overflow = file_read(&pb, 1, sizeof pb, fh);
			if (overflow != sizeof pb)
			{
				pcapng_debug0("pcapng_read_packet_block: failed to read packet data");
				*err = file_error(fh);
				return 0;
		...
		if (wblock->data.packet.cap_len > wblock->allocated)
		{
		...
		overflow = file_read((guchar *)(wblock->frame_buffer), 1, wblock->data.packet.cap_len, fh);
		if (overflow != (int)wblock->data.packet.cap_len)
		{
			*err = file_error(fh);
		...
		if ((wblock->data.packet.cap_len % 4) != 0)
		{
			...
			overflow += 4 - (wblock->data.packet.cap_len % 4);
		}
		...
		errno = WTAP_ERR_CANT_READ;
		exploit = block_total_length - (int)sizeof(pcapng_block_header_t) - overflow - (int)sizeof(bh->block_total_length);
		while (exploit > 0)
		{

			overflow = pcapng_read_option(fh, pn, &oh, option_content, sizeof(option_content), err, err_info);
			if (overflow <= 0)
			{
			...
			case (0):
				if (exploit != 0)
				{
					pcapng_debug1("pcapng_read_packet_block: %u bytes after opt_endofopt", exploit);
				}
