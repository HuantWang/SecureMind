                         ...
                         guint16 type, proto_tree *ncp_tree)
{
    ...
    {
        pvalues[buffer].vtype = 0;
        pvalues[buffer].vvalue = 0;
        pvalues[buffer].vlength = 0;
        pvalues[buffer].voffset = 0;
        pvalues[buffer].hfname = 0;
        pvalues[buffer].vdesc = "";
        pvalues[buffer].vstring = NULL;
        pvalues[buffer].mvtype = 0;
        pvalues[buffer].vflags = 0;
    }
        ...
        case NCP_SERVICE_REQUEST:
            ...
            if (nds_frag == 0xffffffff)
            {
                ...
                for (buffer = 0; buffer < 9; buffer++)
                {
                    ...
                    case VTYPE_UINT8:
                        proto_tree_add_uint_format(ncp_tree, pvalues[buffer].hfname, tvb, pvalues[buffer].voffset,
                                                   pvalues[buffer].vlength, pvalues[buffer].vtype, "NDS Parameter not defined %u",
                                                   pvalues[buffer].vtype);
                        break;
                    ...
                    case VTYPE_UINT32:
                        proto_tree_add_uint_format(ncp_tree, pvalues[buffer].hfname, tvb, pvalues[buffer].voffset,
                                                   pvalues[buffer].vlength, pvalues[buffer].vvalue, pvalues[buffer].vdesc,
                                                   pvalues[buffer].vvalue);
                        break;

                    case VTYPE_STRING:
                        proto_tree_add_string_format(ncp_tree, pvalues[buffer].hfname, tvb, pvalues[buffer].voffset,
                                                     pvalues[buffer].vlength, pvalues[buffer].vstring, pvalues[buffer].vdesc, pvalues[buffer].vstring);
                        if (pvalues[buffer].mvtype == MVTYPE_ATTR_REQUEST)
                        {
                            if (check_col(pinfo->cinfo, COL_INFO))
                            {
                                if (!added_arrow)
                                ...
                                col_append_str(pinfo->cinfo, COL_INFO, pvalues[buffer].vstring);
                            }
                        }
                        break;

                    case VTYPE_BITFIELD:
                        process_bitfield(ncp_tree, tvb, &pvalues[buffer]);
                        break;

                    case VTYPE_MULTIVALUE_UINT32:
                        process_multivalues(ncp_tree, tvb, pinfo, &pvalues[buffer]);
                        if (pvalues[buffer].mvtype == MVTYPE_PROC_ENTRY_SPECIFIERS)
                        {

                            g_strlcpy(global_object_name, mv_resolve_name_string, 256);
                        }
                        if (pvalues[buffer].mvtype == MVTYPE_ATTR_REQUEST || pvalues[buffer].mvtype == MVTYPE_PROC_ENTRY_SPECIFIERS || pvalues[buffer].mvtype == MVTYPE_PROCESS_ITERATOR)
                        {

                            if (check_col(pinfo->cinfo, COL_INFO))
                            {
                                col_append_str(pinfo->cinfo, COL_INFO, pvalues[buffer].vstring);
                                if (pvalues[buffer].mvtype != MVTYPE_PROCESS_ITERATOR)
                                {
                                    if (!added_arrow)
                                    {
                                        col_append_str(pinfo->cinfo, COL_INFO, " -> ");
                    ...
                    case VTYPE_BYTES:
                        proto_tree_add_bytes(ncp_tree, pvalues[buffer].hfname, tvb, pvalues[buffer].voffset,
                                             pvalues[buffer].vlength, tvb_get_ptr(tvb, pvalues[buffer].voffset, pvalues[buffer].vvalue));
                        break;

                    case VTYPE_BOOLEAN:
                        proto_tree_add_item(ncp_tree, pvalues[buffer].hfname, tvb, pvalues[buffer].voffset, pvalues[buffer].vlength, TRUE);
                        break;

                    default:
                        proto_tree_add_uint_format(ncp_tree, hf_nds_p1type, tvb, pvalues[buffer].voffset,
                                                   pvalues[buffer].vlength, pvalues[buffer].vtype, "NDS Parameter not defined %u",
                                                   pvalues[buffer].vtype);
                        break;
                    }
