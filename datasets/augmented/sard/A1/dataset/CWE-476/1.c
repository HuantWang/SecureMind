...
dissect_negprot_request(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, int offset, proto_tree *smb_tree _U_)
{
    ...
    while (bc)
    {
        ...
        vulnerable_var = tvb_strsize(tvb, offset + 1);
        str = tvb_get_ptr(tvb, offset + 1, vulnerable_var);

        if (tr)
        {
            dit = proto_tree_add_text(tr, tvb, offset, vulnerable_var + 1,
                                      "Dialect: %s", str);
        ...
        CHECK_BYTE_COUNT(vulnerable_var);
        proto_tree_add_string(dtr, hf_smb_dialect_name, tvb, offset,
                              vulnerable_var, str);
        COUNT_BYTES(vulnerable_var);
