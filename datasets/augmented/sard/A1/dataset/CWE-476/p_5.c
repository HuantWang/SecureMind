										...
										proto_tree *parent_tree, int offset, int subcmd, guint16 bc)
{
	...
	case 0x00:
		...
		fn = get_unicode_or_ascii_string(tvb, &offset, si->unicode, &overflow, FALSE, FALSE, &bc);
		CHECK_STRING_TRANS(fn);
		proto_tree_add_string(tree, hf_smb_file_name, tvb, offset, overflow,
							  fn);
		COUNT_BYTES_TRANS(overflow);
	...
	case 0x01:
		...
		proto_tree_add_string(tree, hf_smb_search_pattern, tvb, offset, overflow,
							  fn);
		COUNT_BYTES_TRANS(overflow);
	...
	case 0x0d:
		...
		fn = get_unicode_or_ascii_string(tvb, &offset, si->unicode, &overflow,
										 FALSE, FALSE, &bc);
		CHECK_STRING_TRANS(fn);
		proto_tree_add_string(tree, hf_smb_dir_name, tvb, offset, overflow,
							  fn);
		COUNT_BYTES_TRANS(overflow);
