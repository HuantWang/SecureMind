static pa(proto_tree *tree, tvbuff_t *tvb, int offset,
                         guint32 tag_len, gboolean vs)
{
  ...
  proto_tree_add_uint_format(cap_tree, ampduparam_mpdu, tvb, offset, 1, capability, decode_numeric_bitfield(capability, 0x03, 8, "Maximum Rx A-MPDU Length: %%04.0f [Bytes]"), pow(2, 13 + (capability & 0x3)) - 1);
