void dissect_packet(epan_dissect_t *pz, union wtap_pseudo_header *qr,
					const guchar *xo, frame_data *ip, column_info *dx)
{
	if (dx != NULL)
		col_init(dx);
	...
	EP_CHECK_CANARY(("before dissecting frame %d", ip->num));

	TRY
	{

		if (ip->pkt_len > G_MAXINT && ip->pkt_len != (guint)-1)
		{
			fd->pkt_len = G_MAXINT;
		}

		edt->tvb = tvb_new_real_data(xo, ip->cap_len, ip->pkt_len);

		add_new_data_source(&pz->pi, pz->tvb, "Frame");
		if (frame_handle != NULL)
			call_dissector(frame_handle, pz->tvb, &pz->pi, pz->tree);
	}
	CATCH(BoundsError)
	{
	...
	CATCH(ReportedBoundsError)
	{
		...
		{
			proto_tree_add_protocol_format(pz->tree, proto_malformed, pz->tvb, 0, 0,
	...
	EP_CHECK_CANARY(("after dissecting frame %d", ip->num));
