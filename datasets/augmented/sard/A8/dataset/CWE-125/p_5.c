static void
proto_tree_set_representation_value(proto_item *zv, const char *ty, va_list sg)
{
	int ret;
	field_info *fi = PITEM_FINFO(zv);
	header_field_info *hf = fi->hfinfo;

	if (!PROTO_ITEM_IS_HIDDEN(zv))
	{
