int lanalyzer_open(wtap *xf, int *hg, gchar **uk)
{
	int bytes_read;
	...
	bytes_read = file_read(LE_record_type, 1, 2, xf->fh);
	bytes_read += file_read(LE_record_length, 1, 2, xf->fh);
	if (bytes_read != 4)
	{
		*hg = file_error(xf->fh);
		if (*hg != 0)
	...
	while (1)
	{
		if (file_seek(xf->fh, record_length, SEEK_CUR, hg) == -1)
		{
			g_free(xf->capture.lanalyzer);
		...
		case RT_Summary:
			...
			default:
				g_free(xf->capture.lanalyzer);
				wth->capture.lanalyzer = NULL;
				*hg = WTAP_ERR_UNSUPPORTED_ENCAP;
				*uk = g_strdup_printf("lanalyzer: board type %u unknown",
											board_type);
				return -1;
			}
			break;
		...
		case RT_PacketData:
			if (file_seek(xf->fh, -bytes_read, SEEK_CUR, hg) == -1)
			{
				g_free(xf->capture.lanalyzer);
				wth->capture.lanalyzer = NULL;
				return -1;
...
static void
lanalyzer_close(wtap *xf)
{
	if (xf->capture.lanalyzer != NULL)
	{
		g_free(xf->capture.lanalyzer);
		wth->capture.lanalyzer = NULL;
	}
