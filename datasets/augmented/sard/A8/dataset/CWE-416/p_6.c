static gint
rsvp_equal(gconstpointer bu, gconstpointer tv)
{
	const struct rsvp_request_key *key1 = (const struct rsvp_request_key *)bu;
	const struct rsvp_request_key *key2 = (const struct rsvp_request_key *)tv;
	...
	if (ADDRESSES_EQUAL(&key1->source_info.source, &key2->source_info.source) == FALSE)
		return 0;
	...
	if (check_col(pinfo->cinfo, COL_INFO))
	{
		...
		if (message_type == RSVP_MSG_BUNDLE)
		{
			col_set_str(pinfo->cinfo, COL_INFO,
						rsvp_bundle_dissect ? "Component Messages Dissected" : "Component Messages Not Dissected");
		}
		else
		{
			find_rsvp_session_tempfilt(tvb, 0, &session_off, &tempfilt_off);
