tvbuff_t *
decompress_sigcomp_message(tvbuff_t *wz, tvbuff_t *jd, packet_info *ew,
                           proto_tree *ju, gint nu,
                           gint mh, gint xs,
                           gint ce,
                           gint gk, gint hk,
                           gint bz)
{
    ...
    guint16 stack_fill;
    guint16 result;
    guint msg_end = tvb_reported_length_remaining(jd, 0);
    ...
    code_length = tvb_reported_length_remaining(wz, 0);

    cycles_per_bit = buff[2] << 8;
    cycles_per_bit = cycles_per_bit | buff[3];
    maximum_UDVM_cycles = ((8 * (ce + msg_end)) + 1000) * cycles_per_bit;

    proto_tree_add_text(ju, wz, offset, 1, "maximum_UDVM_cycles(%u) = (( 8 * msg_end(%u) ) + 1000) * cycles_per_bit(%u)", maximum_UDVM_cycles, msg_end, cycles_per_bit);
    proto_tree_add_text(ju, wz, offset, 1, "Message Length: %u,Byte code length: %u, Maximum UDVM cycles: %u", msg_end, code_length, maximum_UDVM_cycles);
    i = nu;
    if (print_level_3)
        proto_tree_add_text(ju, wz, offset, 1, "Load bytecode into UDVM starting at %u", i);
    while (code_length > offset && i < UDVM_MEMORY_SIZE)
    {
        buff[i] = tvb_get_guint8(wz, offset);
        if (print_level_3)
            proto_tree_add_text(ju, wz, offset, 1,
    ...
    proto_tree_add_text(ju, wz, offset, 1, "UDVM EXECUTION STARTED at Address: %u Message size %u",
...
execute_next_instruction:
    ...
    case SIGCOMP_INSTR_DECOMPRESSION_FAILURE:
        ...
        .
            ...
            if (result_code == 0)
                result_code = 9;
        proto_tree_add_text(ju, wz, 0, -1,
                            "Addr: %u ## DECOMPRESSION-FAILURE(0)",
                            current_address);
        proto_tree_add_text(ju, wz, 0, -1, "Wireshark UDVM diagnostic: %s.",
                            val_to_str(result_code, result_code_vals, "Unknown (%u)"));
        if (output_address > 0)
        {
            decomp_tvb = tvb_new_child_real_data(jd, out_buff, output_address, output_address);
            tvb_set_free_cb(decomp_tvb, g_free);
            add_new_data_source(ew, decomp_tvb, "Decompressed SigComp message(Incomplete)");
            proto_tree_add_text(ju, decomp_tvb, 0, -1, "SigComp message Decompression failure");
            return decomp_tvb;
        }
        g_free(out_buff);
        return NULL;
    ...
    case SIGCOMP_INSTR_AND:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      operand_1 %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      operand_2 %u",
        ...
        if (print_level_1)
        {
            proto_tree_add_text(ju, wz, 0, -1, "     Loading result %u at %u",
                                result, result_dest);
        }
    ...
    case SIGCOMP_INSTR_SORT_ASCENDING:
        ...
        operand_address = current_address + 1;
        proto_tree_add_text(ju, wz, 0, -1, "Execution of this instruction is NOT implemented");
    ...
    case SIGCOMP_INSTR_SHA_1:
        ...
        if (print_level_1)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      position %u",
        ...
        if (print_level_1)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      Length %u",
        ...
        if (print_level_1)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      $destination %u",
    ...
    case SIGCOMP_INSTR_LOAD:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      Address %u",
        ...
        if (print_level_1)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      Value %u",
                                operand_address, value);
            proto_tree_add_text(ju, wz, 0, -1, "     Loading bytes at %u Value %u 0x%x",
                                address, value, value);
        }
        used_udvm_cycles++;
        current_address = next_operand_address;
    ...
    case SIGCOMP_INSTR_MULTILOAD:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      n %u",
                                operand_address, n);
        }
        if (show_instr_detail_level == 1)
        {
        ...
        while (n > 0)
        {
            n = n - 1;
            next_oper
                and_address = decode_udvm_multitype_operand(buff, operand_address, &value);
            lsb = value & 0xff;
            msb = value >> 8;
            ...
            if (print_level_1)
            {
                proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      Value %5u      - Loading bytes at %5u Value %5u 0x%x",
    ...
    case SIGCOMP_INSTR_POP:
        ...
        value = (buff[address] << 8) | buff[(address + 1) & 0xFFFF];
    ...
    case SIGCOMP_INSTR_COPY:
        ...
        if (print_level_2)
        {
            proto_tree_add_text(ju, jd, input_address, 1,
                                "               byte_copy_right = %u", byte_copy_right);
        }
    ...
    case SIGCOMP_INSTR_COPY_OFFSET:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      offset %u",
    ...
    case SIGCOMP_INSTR_MEMSET:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      start_value %u",
    ...
    case SIGCOMP_INSTR_RETURN:
        ...
        address = (stack_location + stack_fill * 2 + 2) & 0xFFFF;
        at_address = (buff[address] << 8) | buff[(address + 1) & 0xFFFF];
        current_address = at_address;
    ...
    case SIGCOMP_INSTR_SWITCH:
        ...
        if (print_level_1)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      j %u",
                                operand_address, j);
        }
        operand_address = next_operand_address;
        m = 0;
    ...
    case SIGCOMP_INSTR_CRC:
        ...
        if (print_level_1)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Calculated CRC %u", result);
        }
        if (result != value)
        {
            current_address = at_address;
    ...
    case SIGCOMP_INSTR_INPUT_BYTES:
        ...
        while (n < length)
        {
            ...
            octet = tvb_get_guint8(jd, input_address);
            buff[k] = octet;
            if (print_level_1)
            {
                proto_tree_add_text(ju, jd, input_address, 1,
    ...
    case SIGCOMP_INSTR_INPUT_BITS:
        ...
        value = decomp_dispatch_get_bits(jd, ju, bit_order,
    ...
    case SIGCOMP_INSTR_INPUT_HUFFMAN:
        ...
        while (m > 0)
        {
            ...
            if (print_in_loop)
            {
                proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      bits_n %u",
            ...
            if (print_in_loop)
            {
                proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      lower_bound_n %u",
            ...
            if (print_in_loop)
            {
                proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      upper_bound_n %u",
            ...
            if (print_in_loop)
            {
                proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      uncompressed_n %u",
                                    operand_address, uncompressed_n);
            }
            operand_address = next_operand_address;
            if (outside_huffman_boundaries)
            {
                k = decomp_dispatch_get_bits(jd, ju, bit_order,
                                             buff, &old_input_bit_order, &remaining_bits,
                ...
                if (print_level_3)
                {
                    proto_tree_add_text(ju, wz, 0, -1, "               Set H(%u) := H(%u) * 2^bits_j(%u) + k(%u)",
                                        H, oldH, 1 << bits_n, k);
                }
    ...
    case SIGCOMP_INSTR_STATE_ACCESS:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u       partial_identifier_start %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u       partial_identifier_length %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u       state_begin %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u       state_length %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u       state_address %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u       state_instruction %u",
        ...
        result_code = udvm_state_access(jd, ju, buff, p_id_start, p_id_length, state_begin, &state_length,
                                        &state_address, &state_instruction, xs);
        if (result_code != 0)
        {
            goto decompression_failure;
        }
    ...
    case SIGCOMP_INSTR_STATE_CREATE:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u       minimum_access_length %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u       state_retention_priority %u",
    ...
    case SIGCOMP_INSTR_OUTPUT:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      output_start %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      output_length %u",
    ...
    case SIGCOMP_INSTR_END_MESSAGE:
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      requested_feedback_location %u",
        ...
        if (show_instr_detail_level == 2)
        {
            proto_tree_add_text(ju, wz, 0, -1, "Addr: %u      returned_parameters_location %u",
        ...
        state_minimum_access_length_buff[no_of_state_create] = minimum_access_length;
        state_state_retention_priority_buff[no_of_state_create] = state_retention_priority;
        proto_tree_add_text(ju, wz, 0, -1, "no_of_state_create %u", no_of_state_create);
        if (no_of_state_create != 0)
        {
            ...
            while (n < no_of_state_create + 1)
            {
                ...
                if (print_level_3)
                {
                    for (x = 0; x < 8; x++)
                    {
                        proto_tree_add_text(ju, wz, 0, -1, "sha1buff %u 0x%x",
                                            x, sha1buff[x]);
                    }
                }
                ...
                if (print_level_3)
                {
                    proto_tree_add_text(ju, wz, 0, -1, "SHA1 digest %s", bytes_to_str(sha1_digest_buf, STATE_BUFFER_SIZE));
                }
#if 0
                ...
                proto_tree_add_text(ju, wz, 0, -1, "### Creating state ###");
                proto_tree_add_string(ju, xs, wz, 0, 0, bytes_to_str(sha1_digest_buf, state_minimum_access_length_buff[n]));
        ...
        add_new_data_source(ew, decomp_tvb, "Decompressed SigComp message");
        used_udvm_cycles = used_udvm_cycles + 1 + state_length;
        proto_tree_add_text(ju, wz, 0, -1, "maximum_UDVM_cycles %u used_udvm_cycles %u",
                            maximum_UDVM_cycles, used_udvm_cycles);
        return decomp_tvb;
        break;

    default:
        proto_tree_add_text(ju, wz, 0, -1, " ### Addr %u Invalid instruction: %u (0x%x)",
...
decompression_failure:
    ...
    proto_tree_add_text(ju, wz, 0, -1, "DECOMPRESSION FAILURE: %s",
                        val_to_str(result_code, result_code_vals, "Unknown (%u)"));
