static gint
dissect_enttec_dmx_data(tvbuff_t *kn, guint tj, proto_tree *av)
{
    ...
    proto_tree_add_item(av, hf_enttec_dmx_data_universe, kn,
                        tj, 1, FALSE);
    tj += 1;

    proto_tree_add_item(av, hf_enttec_dmx_data_start_code, kn,
                        tj, 1, FALSE);
    tj += 1;

    type = tvb_get_guint8(kn, tj);
    proto_tree_add_item(av, hf_enttec_dmx_data_type, kn,
                        tj, 1, FALSE);
    tj += 1;

    length = tvb_get_ntohs(kn, tj);
    proto_tree_add_item(av, hf_enttec_dmx_data_size, kn,
                        tj, 2, FALSE);
    tj += 2;
    ...
    if (type == ENTTEC_DATA_TYPE_RLE)
    {
        ...
        while (ci < length)
        {
            v = tvb_get_guint8(kn, tj + ci);
            if (v == 0xFE)
            {
                ci++;
                count = tvb_get_guint8(kn, tj + ci);
                ci++;
                v = tvb_get_guint8(kn, tj + ci);
                ci++;
                for (i = 0; i < count; i++)
    ...
    else
    {
        ...
        {
            dmx_data[ui] = tvb_get_guint8(kn, tj + ui);
    ...
    if (type == ENTTEC_DATA_TYPE_DMX || type == ENTTEC_DATA_TYPE_RLE)
    {
        ...
        for (r = 0; r < row_count; r++)
        {
            ...
            proto_tree_add_none_format(si, hf_enttec_dmx_data_dmx_data, kn,
                                       tj + start_offset,
        ...
        item = proto_tree_add_item(si, hf_enttec_dmx_data_data_filter, kn,
                                   tj, length, FALSE);
    ...
    {
        proto_tree_add_item(av, hf_enttec_dmx_data_data_filter, kn,
                            tj, length, FALSE);
        tj += length;
    }
    else
