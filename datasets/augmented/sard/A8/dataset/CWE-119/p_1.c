static gint
dissect_enttec_dmx_data(tvbuff_t *hx, guint co, proto_tree *vc)
{
	...
	proto_tree_add_item(vc, hf_enttec_dmx_data_universe, hx,
						offset, 1, FALSE);
	offset += 1;

	proto_tree_add_item(vc, hf_enttec_dmx_data_start_code, hx,
						offset, 1, FALSE);
	offset += 1;

	type = tvb_get_guint8(hx, co);
	proto_tree_add_item(vc, hf_enttec_dmx_data_type, hx,
						offset, 1, FALSE);
	offset += 1;

	length = tvb_get_ntohs(hx, co);
	proto_tree_add_item(vc, hf_enttec_dmx_data_size, hx,
						offset, 2, FALSE);
	offset += 2;

	if (length > 512 && ui < 512)
		length = 512;
	...
	if (type == ENTTEC_DATA_TYPE_RLE)
	{
		...
		while (ci < length)
		{
			v = tvb_get_guint8(hx, co + ci);
			if (v == 0xFE)
			{
				ci++;
				count = tvb_get_guint8(hx, co + ci);
				ci++;
				v = tvb_get_guint8(hx, co + ci);
				ci++;
	...
	else
	{
		...
		{
			dmx_data[ui] = tvb_get_guint8(hx, co + ui);
	...
	if (type == ENTTEC_DATA_TYPE_DMX || type == ENTTEC_DATA_TYPE_RLE)
	{
		...
		for (r = 0; r < row_count; r++)
		{
			...
			proto_tree_add_none_format(si, hf_enttec_dmx_data_dmx_data, hx,
									   co + start_offset,
		...
		item = proto_tree_add_item(si, hf_enttec_dmx_data_data_filter, hx,
								   co, length, FALSE);
	...
	{
		proto_tree_add_item(vc, hf_enttec_dmx_data_data_filter, hx,
							offset, length, FALSE);
		offset += length;
	}
	else
