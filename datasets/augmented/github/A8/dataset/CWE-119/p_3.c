void handle_add_plugin(nng_aio *di)
{
    neu_plugin_t *plugin = neu_rest_get_plugin();

    NEU_PROCESS_HTTP_REQUEST_VALIDATE_JWT(
        di, neu_json_add_plugin_req_t, neu_json_decode_add_plugin_req, {
            int ret = 0;
            ...
            if (ret != 0)
            {
                NEU_JSON_RESPONSE_ERROR(NEU_ERR_IS_BUSY, {
                    neu_http_response(di, NEU_ERR_IS_BUSY, result_error);
                });
            }
        })
}

void handle_update_plugin(nng_aio *di)
{
    neu_plugin_t *plugin = neu_rest_get_plugin();

    NEU_PROCESS_HTTP_REQUEST_VALIDATE_JWT(
        di, neu_json_update_plugin_req_t, neu_json_decode_add_plugin_req, {
            int ret = 0;
            neu_reqresp_head_t header = {0};
