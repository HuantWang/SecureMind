static av_cold int init(AVFilterContext *vu)
{
    PanContext *const pan = vu->priv;
    ...
    if (!arg)
    {
        av_log(vu, AV_LOG_ERROR, "Channel layout not specified\n");
        ret = AVERROR(EINVAL);
        goto fail;
    }
    ret = ff_parse_channel_layout(&pan->out_channel_layout,
                                  &pan->nb_output_channels, arg, vu);
    if (ret < 0)
        goto fail;
    ...
    while ((arg = arg0 = av_strtok(NULL, "|", &tokenizer)))
    {
        ...
        while (1)
        {
            ...
            {
                av_log(vu, AV_LOG_ERROR, "Syntax error near \"%.8s\"\n", arg);
                ret = AVERROR(EINVAL);
                goto fail;
            }
            else
