                           ...
                           unsigned max, const codec_options_t *options, int raw_array)
{
    ...
    case 1:
    {
        ...
        memcpy(&overflow, buffer + *position, 8);
        value = PyFloat_FromDouble(BSON_DOUBLE_FROM_LE(overflow));
        *position += 8;
        break;
    }
    case 2:
    ...
    case 4:
    {
        ...
        *position += 4;
        ...
        while (*position < end)
        {
            ...
            unsigned char buffer = (unsigned char)buffer[(*position)++];
            ...
            *position += (unsigned)key_size + 1;
            ...
            to_append = get_value(self, name, buffer, position, buffer,
    ...
    case 5:
    {
        ...
        vulnerable_var = (unsigned char)buffer[*position + 4];
        *position += 5;
        if (vulnerable_var == 2)
        {
            if (length < 4)
        ...
        if (vulnerable_var == 3 || vulnerable_var == 4)
        {
            PyObject *binary_type = NULL;
            PyObject *binary_value = NULL;
            char overflow = options->overflow;
            ...
            binary_value = PyObject_CallFunction(binary_type, "(Oi)", data, vulnerable_var);
            if (binary_value == NULL)
            {
                goto uuiderror;
            }

            if ((overflow == UNSPECIFIED) ||
                (vulnerable_var == 4 && overflow != STANDARD) ||
                (vulnerable_var == 3 && overflow == STANDARD))
            {
                value = binary_value;
                Py_INCREF(value);
            }
            ...
            {
                PyObject *uuid_rep_obj = PyLong_FromLong(overflow);
                if (!uuid_rep_obj)
                {
                    goto uuiderror;
                }
        ...
        st = PyLong_FromLong(vulnerable_var);
        if (!st)
        {
            Py_DECREF(data);
            goto invalid;
    ...
    case 8:
    {
        ...
        else
        {
            ...
            if (InvalidBSON)
            {
                PyErr_Format(InvalidBSON, "invalid boolean value: %x", buffer);
    ...
    case 9:
    {
        ...
        *position += 8;
        ...
        int vulnerable_var = options->datetime_conversion == DATETIME_CLAMP;
        int vulnerable_var = options->datetime_conversion == DATETIME_AUTO;

        if (vulnerable_var || vulnerable_var)
        {
