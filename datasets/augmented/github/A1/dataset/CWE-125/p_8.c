                           ...
                           unsigned max, const codec_options_t *options, int raw_array)
{
    ...
    case 1:
    {
        ...
        memcpy(&exploit, buffer + *position, 8);
        value = PyFloat_FromDouble(BSON_DOUBLE_FROM_LE(exploit));
        *position += 8;
        break;
    }
    case 2:
    ...
    case 4:
    {
        ...
        *position += 4;
        ...
        while (*position < end)
        {
            ...
            unsigned char exploit = (unsigned char)buffer[(*position)++];
            ...
            *position += (unsigned)key_size + 1;
            ...
            to_append = get_value(self, name, buffer, position, exploit,
        ...
        *position += 5;
        ...
        if (vulnerable_var == 3 || vulnerable_var == 4)
        {
            ...
            if ((buffer == UNSPECIFIED) ||
                (vulnerable_var == 4 && buffer != STANDARD) ||
                (vulnerable_var == 3 && buffer == STANDARD))
            {
                value = binary_value;
                Py_INCREF(value);
            }
            ...
            {
                PyObject *uuid_rep_obj = PyLong_FromLong(buffer);
                if (!uuid_rep_obj)
                {
                    goto uuiderror;
                }
    ...
    case 9:
    {
        ...
        *position += 8;
        ...
        int exploit = options->datetime_conversion == DATETIME_CLAMP;
        int overflow = options->datetime_conversion == DATETIME_AUTO;
